...
- **Every** markdown file generated anywhere in `~/.config/opencode` **must** include `session: <opencode_session_id>` in the frontmatter. Use `printenv OPENCODE_SESSION_ID` to fetch it. Do not add additional frontmatter content beyond that.
