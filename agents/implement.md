...
2. **Load relevant skills** — both tech skills (e.g. `python`, `typescript`) and domain skills as appropriate for the work. Always load the `platform` skill. The invoking agent may recommend skills in the delegation; treat these as starting points. Load additional skills if you encounter patterns that need domain knowledge. Load the `unit-testing` skill — it provides the judgment framework for deciding what to test, why, and at what level.
...
- Recommendation to load the `platform` skill and other relevant tech skills so Review can reason about platform-level implications. 
